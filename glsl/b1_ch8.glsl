...
vec3 random_in_unit_sphere() {
    ...
    //     return p;
    // }
    float x = random(vec3(gl_FragCoord.xy, 1));
    float y = random(vec3(gl_FragCoord.xy, 2));
    float z = random(vec3(gl_FragCoord.xy, 3));
    return vec3(x, y, z) * 2.0 - 1.0;
}

vec3 random_unit_vector() {
    float a = random(vec3(gl_FragCoord.xy, 1)) * 2.0 * pi;
    float z = random(vec3(gl_FragCoord.xy, 2)) * 2.0 - 1.0;
...
vec3 ray_color_depth(ray r, int depth) {
    ...
    while (depth > 0) {
        ...
        t = world_hit(ir, 0.0001, 100000.0, rec);
        if (t <= 0.0)
            break;

        // 漫反射, 围绕法线周围随机一个点作为反射方向
        // target = rec.p + rec.normal + random_in_unit_sphere();
        target = rec.p + rec.normal + random_unit_vector();

        // 构造反射之后的射线
        ir = ray_new(rec.p, target - rec.p);
        attenuation = attenuation * 0.5;
    }

    // scale t to [0, 1]
    t = 0.5 * (r.direction.y + 1.0);
    return attenuation * mix(vec3(1.0, 1.0, 1.0), vec3(0.5, 0.7, 1.0), t);
}
...
void main()
{
    ...
    vec3 color = ray_color_depth(r, 20);
